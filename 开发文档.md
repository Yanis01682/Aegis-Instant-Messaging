...
当前后端测试位于 `backend/tests/`，覆盖认证、数据库、聊天、好友和群聊高级能力。后续维护建议：

1. 为 `PUT /auth/status` 补充后端实现或删除前端调用。
2. 为 WebSocket 鉴权增加 Token 校验，避免仅凭 `user_id` 建立连接。
3. 为好友关系、会话成员、置顶记录增加唯一约束，防止重复数据。
4. 将启动时手写迁移逐步迁移到 Alembic 等正式迁移工具。
5. 多媒体文件建议增加清理机制，删除消息或会话时同步删除 `/uploads` 中无用文件。
